#' Plastics in Snow
#'
#' Measurements of lengths and diameters of plastic microfibers found in the snow in the Arctic and Europe.
#'
#' From the authors: "Microplastics (MPs) are ubiquitous, and considerable quantities prevail even in the Arctic; however, there are large knowledge gaps regarding pathways to the North. To assess whether atmospheric transport plays a role, we analyzed snow samples from ice floes in Fram Strait."
#'
#'
#' @format A data frame with 237 observations of 4 variables.
#' \describe{
#'   \item{colour}{Color of microfiber - inconsistently reported.}
#'   \item{length}{Length of plastic microfiber, in micrometers.}
#'   \item{diameter}{Diameter of plastic microfiber, in micrometers.}
#'   \item{region}{Arctic or Europe}
#' }
#'
#' @source Bergmann M, Mützel S, Primpke S, Tekman MB, Trachsel J, Gerdts G. White and wonderful? Microplastics prevail in snow from the Alps to the Arctic. Sci Adv. 2019 Aug 14;5(8):eaax1157. doi: 10.1126/sciadv.aax1157. PMID: 31453336; PMCID: PMC6693909.
"plastics"
